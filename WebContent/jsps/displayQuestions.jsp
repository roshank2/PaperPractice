...
<body>
	...
	<h2 style="margin: 0 0 32px 28px;">Select question to <%=request.getParameter("act") %></h2>
		...
		%>
		<b><%=i%>. &nbsp;</b><a
			href="${pageContext.request.contextPath}/jsps/modifyQuestion.jsp?topic_id=<%=request.getParameter("topic_id")%>&ques_id=<%=set.getInt("ques_id")%>&tname=<%=request.getParameter("tname") %>"><%=set.getString("question")%></a><br />
		<%
			i++;
		%>
		<%
		...
		%>
		<b><%=i%>. &nbsp;</b><a
			href="${pageContext.request.contextPath}/jsps/deleteQuestion.jsp?ques_id=<%=set.getInt("ques_id")%>"><%=set.getString("question")%></a><br />
		<%
			i++;
		%>
		<%
