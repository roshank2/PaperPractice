<jsp:useBean id="formHandler" class="bean.User" scope="request" />
<html>
<head>
<meta charset="ISO-8859-1">
<title>User Registration</title>
<link rel="stylesheet" href="styles.css">
<link
	href="https://fonts.googleapis.com/css2?family=Alegreya:wght@900&family=B612:wght@700&family=Vollkorn&display=swap"
	rel="stylesheet">
</head>
<body>
<body>
	<div class="split left">
		<div class="centered">
			<h1
				style="font-family: 'Alegreya', serif; font-size: 4em; margin-bottom: 0.15em;">Practice</h1>
			<h2 style="font-family: 'B612', sans-serif; font-size: 2em;">like
				you've never won.</h2>
			<h1
				style="font-family: 'Alegreya', serif; font-size: 4em; margin-bottom: 0.15em;">Perform</h1>
			<h2 style="font-family: 'B612', sans-serif; font-size: 2em;">like
				you've never lost.</h2>
		</div>
	</div>

	<div class="split right">
		<form action="processRegistration.jsp" method="post">
			<label for="uname"><b>Name</b></label><sup>*</sup><input type="text"
				placeholder="Enter your full name" name="uname" /><br>
			<div class="warning"><%=formHandler.getErrorMsg("uname")%></div>
			<label for="uemail"><b>Email-ID</b></label><sup>*</sup> <input
				type="text" placeholder="Enter email-id" name="uemail" /><br>
			<div class="warning"><%=formHandler.getErrorMsg("uemail")%></div>
			<label for="upass"><b>Password</b></label><sup>*</sup> <input
				type="password" placeholder="Enter Password" name="upass" /><br>
			<div class="warning"><%=formHandler.getErrorMsg("upass")%></div>
			<label for="upass"><b>Confirm Password</b></label><sup>*</sup> <input
				type="password" placeholder="Enter Password" name="ucpass" />
			<div class="warning"><%=formHandler.getErrorMsg("ucpass")%></div>
			<button type="submit">Register</button>
			<div class="warning">
				<sup>*</sup> Required Fields
			</div>
		</form>
		<div class="containerdash">
			<b>Already registered?</b> <a href="index.html">Login here</a>
		</div>
	</div>
</body>
</html>


